...
player_y = SCREEN_HEIGHT - 20 - player_height  # Spawn on the ground platform
player_speed = 5
...
def generate_platforms():
    platforms = [pygame.Rect(0, SCREEN_HEIGHT - 20, SCREEN_WIDTH, 20)] #Ground platform
    while len(platforms) < math.ceil(SCREEN_HEIGHT/player_height):
        x = random.randint(0, SCREEN_WIDTH - 200)
        y = random.randint(150, SCREEN_HEIGHT - 100)  # Adjusted to ensure platforms are reachable
        new_platform = pygame.Rect(x, y, random.randint(100, 200), 20)
        if all(not new_platform.colliderect(p) for p in platforms):  # Ensure no overlap
            platforms.append(new_platform)
    return sorted(platforms, key=lambda p: p.top, reverse=True)  # Sort by y-position (topmost first)
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if message_displayed and event.key == pygame.K_r:
                    player_x, player_y = random.randint(100, SCREEN_WIDTH - player_width), 980 - player_height # Reset player position
                    player_velocity_y = 0
                    is_jumping = False
                    message_displayed = False
                    platforms = generate_platforms()  # Regenerate platforms
        ...
        # Check if the player is standing on the topmost platform
        ...
        if player_y > SCREEN_HEIGHT:
            player_x, player_y = 100, 500  # Reset position
            player_velocity_y = 0
            is_jumping = False
            platforms = generate_platforms()  # Regenerate platforms
